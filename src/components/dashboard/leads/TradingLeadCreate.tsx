...
export default function TradingLeadCreate({ onBack, onSave }: { onBack: () => void; onSave: (lead: TradingLead) => void }) {
  ...
  const submit = () => {
    if (!form.firstName.trim() || !form.mobile.trim() || !form.note.trim()) {
      setError("Customer name, mobile and call note are required.");
      return;
    }
  ...
  if (saved) {
    return (
      <div className="rounded-[2.5rem] border border-slate-100 bg-white p-10 text-center shadow-sm">
        ...
        <h3 className="text-3xl font-black tracking-tight text-primary">Trading Lead Added</h3>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <p className="text-[10px] font-black uppercase tracking-[0.25em] text-slate-400">Trading Lead</p>
          <h2 className="text-2xl font-black text-primary">Create Calling / Support Lead</h2>
        </div>
      </div>
      ...
      <div className="rounded-[2.5rem] border border-slate-100 bg-white p-8 shadow-sm">
        ...
        <div className="grid gap-4 md:grid-cols-3">
          ...
          <Field label="Assign Telecaller" options={telecallers.map((caller) => caller.id)} value={form.assignedTo} onChange={(value) => update("assignedTo", value)} />
          ...
          <Field label="Trading Interest" value={form.tradingInterest} onChange={(value) => update("tradingInterest", value)} />
        ...
        <div className="mt-6 flex justify-end">
          <button onClick={submit} className="flex h-11 items-center gap-2 rounded-xl bg-accent px-5 text-xs font-black uppercase tracking-widest text-slate-950">
            <Save size={15} /> Save Trading Lead
          </button>
        </div>
      </div>
    </div>
...
function Field({ label, value, onChange, type = "text", options, multiline }: { label: string; value: string; onChange: (value: string) => void; type?: string; options?: string[]; multiline?: boolean }) {
  return (
