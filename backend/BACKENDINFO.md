...
- Projects, Finance, and HRMS service layers now emit shared notification events for project delivery setup, finance approvals/payments, and employee onboarding/leave/payroll/exit flows through the centralized notifications helper.
...
| 2026-07-24 | Added production notification hooks to the service layer: Projects now emit notifications for project creation, team assignment, milestone completion, task completion, and deadline status changes; Finance emits notifications for client/vendor creation, quotation/invoice/payment status changes, and approval request queueing; HRMS emits notifications for employee onboarding, employee updates/archive, leave decisions, payroll progression, and exit lifecycle changes. Django check passed after the hooks were added. |
| 2026-07-25 | Removed the legacy `LeadWizard.tsx` mock seed bridge and switched it to load and persist leads through the centralized backend `leads-api.ts`; the wizard now hydrates from `GET /api/v1/leads/`, creates leads through the backend, updates the final status through `PUT /api/v1/leads/{id}/`, and no longer relies on local mock lead rows. Synced `PANEL.md` so the panel status now marks Onboarding, Lead, and Accounting as completed. Frontend TypeScript passed after the cutover. |
| 2026-07-25 | Completed a CRM / Client Operations audit pass across `ClientsContacts.tsx`, `FollowUps.tsx`, `LeadAssign.tsx`, `LeadOutcomes.tsx`, `ProjectAgreement.tsx`, and `TelecallerDesk.tsx`: all six screens use centralized service wrappers (`leads-api.ts`, `hrms-api.ts`) with backend load/save flows and no component-level mock/localStorage bridge. Updated `PANEL.md` to mark Client Operations / CRM Module complete. |
| 2026-07-30 | Completed the remaining exposed-module RBAC audit for HRMS, Delivery Projects, and Marketing: added backend permission modules plus view-level 403 checks for module access and sensitive actions, and blocked non-authorized users from HRMS employee/attendance/leave/payroll/exit routes, Delivery Projects list/detail/team/milestone/task/deadline/review routes, and Marketing overview/campaign/source routes. Verified frontend TypeScript passed, HRMS tests passed, Projects tests passed, and Marketing tests passed after clearing the stale local MySQL test database. |
| 2026-08-01 | Hardened Notifications API permissions: authenticated users can read/mark their own or broadcast notifications, while create/update notification and communication-job endpoints are restricted to admin-level access via `IsAccountsAdmin`. Verified `apps.notifications` tests pass after the access split. |
| 2026-08-01 | Hardened Accounts admin RBAC: user/session lifecycle endpoints remain available to admin access, but role and permission management endpoints now require super-admin access, and protected system roles cannot be updated by non-super admins. Verified `apps.accounts` tests pass after the split. |
| 2026-08-01 | Synchronized the admin frontend shell with the backend RBAC split: Admin Control now hides the `Roles & Permissions` path for non-super-admin users, keeps the remaining admin tabs aligned with allowed admin views, and page-level guards redirect invalid admin tab opens to a safe fallback. Frontend TypeScript and ESLint passed after the change. |
| 2026-08-01 | Ran a seeded QA smoke verification across CRM, Projects, Finance, Support, and Accounts RBAC with fake users (`qa.admin@crmproduct.local`, `qa.telecaller@crmproduct.local`, `qa.pm@crmproduct.local`): super-admin access returned `200` on roles/leads/projects/finance/support/marketing overview endpoints, CRM lead create/assign returned `201/200`, project create/team assign returned `201/201`, finance client create returned `201`, support ticket create returned `201`, and non-super access to accounts roles and finance overview returned `403`. Fixed the telecaller CRM page gap by adding `leads` to backend CRM page access so the telecaller desk can load assigned leads while generic admin pages remain blocked. |
| 2026-08-01 | Completed the Support Desk audit pass: backend `overview`, `tickets`, ticket detail, and comment routes all enforce support access plus action checks, the frontend shell keeps a restricted fallback for unsupported roles, and no new exposed Support Desk path was found in the page/route scan. |
| 2026-08-07 | Started final production RBAC hardening pass. Tightened Delivery Projects object-level scope so non-super project users only see/access assigned projects: project managers are limited to projects they manage or are assigned to; team leads are limited to their assigned/team projects; project child actions for team assignments, milestones, tasks, deadlines, and performance reviews now use the scoped queryset so direct URL/API access cannot reach unrelated projects. Fixed CRM frontend/backend mismatch by allowing `team_lead` backend access to the Calling Desk page that the dashboard already exposes. Added regression coverage for project-manager assigned-project scoping. Verification passed: Django check, CRM + Projects backend tests (`17` tests), frontend TypeScript, and frontend ESLint. |
| 2026-08-07 | Continued production RBAC hardening for Admin/Accounts. Added backend service-level protection for `super_admin` and `admin` role changes: only a Django superuser or user holding the `super_admin` role can assign or remove protected system roles during account creation or role reassignment. Normal admin users can still manage ordinary users/roles but cannot escalate anyone into admin access or remove protected admin access through direct API calls. Added regression tests for blocked staff-admin escalation/removal and allowed superuser role assignment. Verification passed: Django check, Accounts backend tests (`13` tests), and CRM + Projects backend smoke tests (`17` tests). |
| 2026-08-07 | Completed Finance Control RBAC hardening pass. Backend now blocks normal finance users from Finance Access Policy endpoints and blocks approval-policy mutations unless the user has super/admin finance authority, while preserving finance overview and operational finance access. Added regression coverage proving a finance user can load Finance Control overview but receives `403` for Access Control and approval-policy create attempts. Frontend dashboard accounting guard now uses the accounting module permission map instead of exposing every accounting tab to finance users, so restricted accounting module tabs cannot be opened manually from the CRM shell. Verification passed: Django check, Finance backend tests (`17` tests), frontend TypeScript, and frontend ESLint. |
| 2026-08-07 | Completed Support Desk and HRMS self-access RBAC pass. Support Desk ticket list/detail/comment/update paths now use scoped ticket querysets: super/admin can see all tickets, while support users only see tickets they own, created, were assigned to, or commented on; direct unrelated ticket UUID access returns `404`. HRMS now supports read-only employee self-access for users with an HR profile: employees can view only their own employee profile and payroll/attendance/leave/exit records, while create/edit/delete/approve actions remain `403`; HR/admin access remains unchanged. Frontend dashboard now exposes Employee Directory and Payroll to employee role and hides HRMS write/action controls for non-HR users. Verification passed: Django check, Support + HRMS backend tests (`16` tests), frontend TypeScript, and frontend ESLint. |
| 2026-08-07 | Ran consolidated final RBAC verification after the production hardening pass. Backend system check passed, Accounts + CRM + Projects tests passed (`30` tests), Finance + Support + HRMS tests passed (`33` tests), frontend TypeScript passed, and frontend ESLint passed. Note: the first parallel test attempt hit an existing MySQL `test_crmproduct` database conflict because two test batches created the same test database at the same time; rerunning the affected batch sequentially passed cleanly. |
| 2026-08-07 | Started production readiness checklist. Hardened `settings_production` with fail-fast checks for strong `SECRET_KEY`, explicit `ALLOWED_HOSTS`, explicit `CORS_ALLOWED_ORIGINS`, CSRF trusted origins, secure cookies, SSL redirect, HSTS, and reverse-proxy HTTPS header. Added production environment templates for backend/frontend and a root `PRODUCTION_READINESS_CHECKLIST.md` runbook covering env setup, migrate, collectstatic, seed roles, frontend build, and smoke tests. Added `seed_foundation --skip-admin` so roles/permissions can be seeded without resetting the admin password. Generated and applied pending `notifications.0002` migration so migration check is clean. Verification passed: production `check --deploy` with temporary safe env values, `makemigrations --check --dry-run`, local `migrate`, safe role seed (`13/13` required role codes present), Accounts + Notifications tests (`16` tests), frontend build, frontend lint, collectstatic dry run, and health/db smoke endpoints (`200`). |
